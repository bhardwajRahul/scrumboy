...
export async function loadUserTheme() {
    if (getUser()) {
        try {
            const resp = await apiFetch("/api/user/preferences?key=theme");
            if (resp && resp.value) {
...
export { THEME_STORAGE_KEY, THEME_SYSTEM, THEME_DARK, THEME_LIGHT, THEME_CHANGE_EVENT, getStoredTheme, setStoredTheme, getSystemTheme, getEffectiveTheme, applyTheme, handleThemeChange, initTheme, };
