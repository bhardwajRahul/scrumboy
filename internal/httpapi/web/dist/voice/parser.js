...
import { containsProjectScopeOverride, normalizePhrase, normalizeTitleReference, parseSpokenNumber, stripWrappingQuotes, } from './normalize.js';
...
function optionalEntityTarget(raw) {
    const normalized = normalizePhrase(raw);
    const match = normalized.match(new RegExp(`^${ENTITY_ALIAS_PATTERN}\\s+(.+)$`, "i"));
    return match ? match[1] : normalized;
}
function requiredEntityTarget(raw) {
    const normalized = normalizePhrase(raw);
    const match = normalized.match(new RegExp(`^${ENTITY_ALIAS_PATTERN}\\s+(.+)$`, "i"));
    return match ? match[1] : null;
}
function splitByCommandDelimiter(raw, delimiter) {
    const normalized = normalizePhrase(raw);
