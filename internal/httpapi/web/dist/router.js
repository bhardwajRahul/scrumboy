...
import { hydrateVoiceFlowHandsFreeConfirmationFromServer, hydrateVoiceFlowModeFromServer, VOICE_FLOW_HANDS_FREE_CONFIRMATION_PREFERENCE_KEY, VOICE_FLOW_MODE_PREFERENCE_KEY, } from './core/voiceflow-preferences.js';
...
async function routeOnce() {
    ...
    if (!getAuthStatusChecked()) {
        ...
        if (newUser) {
            ...
            try {
                const modeResp = await apiFetch(`/api/user/preferences?key=${VOICE_FLOW_MODE_PREFERENCE_KEY}`);
                if (modeResp?.value)
                    hydrateVoiceFlowModeFromServer(modeResp.value);
            }
            catch (err) {
                // Ignore errors
            }
