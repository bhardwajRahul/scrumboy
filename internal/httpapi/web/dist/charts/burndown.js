...
function prepareChartData(data, currentSprint, dataIsSprintScoped) {
    let chartData = data
...
export function destroyBurndownChart() {
    if (uplotInstance) {
        try {
 ...
 */
function buildUplotData(chartData, initialScope, initialScopePoints, currentSprint) {
    ...
    const x = Array.from(new Set([startSec, endSec, ...pointBySecond.keys()])).sort((a, b) => a - b);
    const primarySeries = [];
    const idealSeries = [];
...
function computeYExtents(alignedYSeries) {
    const values = [];
...
export function mountBurndownChart(container, data, currentSprint, dataIsSprintScoped) {
    ...
    const initialScope = toNumeric(chartData[0]?.initialScope) ?? 0;
    const initialScopePoints = toNumeric(chartData[0]?.initialScopePoints) != null
        ? toNumeric(chartData[0]?.initialScopePoints)
        : null;
    const { x, primarySeries, idealSeries, visibleCount, usePoints, lastValueIdx, } = buildUplotData(chartData, initialScope, initialScopePoints, currentSprint);
    ...
    const remaining = lastValueIdx != null ? primarySeries[lastValueIdx] : null;
    const totalScope = usePoints && initialScopePoints != null ? initialScopePoints : initialScope;
    const idealPointsPerDay = durationDays > 0 ? totalScope / durationDays : null;
    const durationDaysDisplay = durationDays > 0 ? String(durationDays) : '—';
    const remainingDisplay = remaining != null ? String(remaining) : '—';
    const idealLabel = idealPointsPerDay != null
  ...
  `;
    ...
    const alignedData = [x, primarySeries, idealSeries];
    ...
    const yExtents = computeYExtents(alignedData.slice(1).map((s) => s));
    ...
    try {
        ...
        requestAnimationFrame(() => {
            if (!uplotInstance || typeof uplotInstance.setSize !== 'function')
                return;
