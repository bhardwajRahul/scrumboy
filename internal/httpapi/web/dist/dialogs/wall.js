...
function wallModeButtonState(mode) {
    if (mode === "pan") {
        return {
            pressed: "true",
...
function syncWallCanvasModeUi() {
    ...
    if (wallSurface) {
        ...
        if (mode !== "pan")
            wallSurface.classList.remove("wall-surface--panning");
    }
...
function onWallLocaleChange() {
    if (!getMounted())
        return;
    if (wallDialog)
        hydrateI18n(wallDialog);
    syncWallLocaleState();
}
function syncWallLocaleState() {
    ...
    if (wallTrash)
        wallTrash.setAttribute("alt", t("wall.shell.trashAlt"));
    if (!wallSurface)
        return;
    ...
    if (wallDialog) {
        wallDialog.querySelectorAll(".wall-note-context-menu [data-i18n-count]").forEach((el) => {
