  ...
  if (bootstrapAvailable) {
    ...
    if (authForm) {
      authForm.addEventListener("submit", async (e) => {
        ...
        const password = pwEl.value;
        try {
          const res = await apiFetch<{ requires2fa?: boolean; tempToken?: string; user?: { id: number; email?: string; name?: string } } | null>(
            "/api/auth/login",
...
function render2FAStep(opts: { tempToken: string; user: { id: number; email?: string; name?: string }; next: string }): void {
