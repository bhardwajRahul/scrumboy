import { commandFailure, isCommandFailure, type CommandResult, type ParsedCommandDraft } from './schema.js';
import {
...
function parseCreate(input: string): CommandResult<ParsedCommandDraft> | null {
  ...
  if (!isEntityAlias(match[1])) return null;
...
function parseMove(input: string): CommandResult<ParsedCommandDraft> | null {
  ...
  if (!match) return null;
  if (match[1] && !isEntityAlias(match[1])) return null;
  const id = parseId(match[2]);
  if (isCommandFailure(id)) return id;
...
function parseOpen(input: string): CommandResult<ParsedCommandDraft> | null {
  ...
  if (!match) return null;
  if (match[2] && !isEntityAlias(match[2])) return null;
