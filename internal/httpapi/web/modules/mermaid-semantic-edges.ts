export type MermaidSemanticEdgePair = {
  positive: string;
  negative: string;
};

export type MermaidSemanticEdgeConfig = {
  positiveColor: string;
  negativeColor: string;
  pairs: MermaidSemanticEdgePair[];
};

export const MERMAID_SEMANTIC_EDGES_URL = "/mermaid-semantic-edges.json";

export const DEFAULT_MERMAID_SEMANTIC_EDGE_CONFIG: MermaidSemanticEdgeConfig = {
...
let semanticEdgeConfigPromise: Promise<MermaidSemanticEdgeConfig> | null = null;
...
function isSkippableDiagramLine(line: string): boolean {
...
function parseFlowchartEdge(line: string): { from: string; label: string } | null {
  const trimmed = line.trim();
...
function sanitizeConfig(raw: unknown): MermaidSemanticEdgeConfig | null {
  if (!raw || typeof raw !== "object") {
    return null;
  }
  const value = raw as Record<string, unknown>;
  const positiveColor =
    typeof value.positiveColor === "string" && value.positiveColor.trim() ? value.positiveColor.trim() : null;
  const negativeColor =
    typeof value.negativeColor === "string" && value.negativeColor.trim() ? value.negativeColor.trim() : null;
  if (!positiveColor || !negativeColor || !Array.isArray(value.pairs)) {
    return null;
  }

  const pairs: MermaidSemanticEdgePair[] = [];
...
export async function loadMermaidSemanticEdgeConfig(): Promise<MermaidSemanticEdgeConfig> {
...
export function resetMermaidSemanticEdgeConfigCacheForTests(): void {
...
function matchPair(
  ...
  rightLabel: string,
  pairs: MermaidSemanticEdgePair[],
): { positiveLabel: string; negativeLabel: string } | null {
...
export function buildMermaidSemanticLabelColors(
  source: string,
  config: MermaidSemanticEdgeConfig = DEFAULT_MERMAID_SEMANTIC_EDGE_CONFIG,
): Map<string, string> {
  ...
  const bySource = new Map<string, ParsedEdge[]>();
...
function colorMatchedLabel(label: Element, color: string): void {
  ...
  const ancestorBkg = label.closest(".labelBkg") as HTMLElement | null;
  if (ancestorBkg) {
    ancestorBkg.style.backgroundColor = color;
  }
  for (const bkg of Array.from(group.querySelectorAll<HTMLElement>(".labelBkg"))) {
    bkg.style.backgroundColor = color;
  }
  for (const span of Array.from(group.querySelectorAll<HTMLElement>("span.edgeLabel"))) {
...
export function paintMermaidSemanticLabelBackgrounds(canvas: HTMLElement, labelColors: Map<string, string>): void {
