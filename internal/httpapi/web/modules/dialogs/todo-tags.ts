...
export function resetTodoTagAutocompleteBindings(): void {
...
export function getTagsFromChips(): string[] {
  ...
  return Array.from(chipsContainer.querySelectorAll(".tag-chip")).map(
  ...
  chipsContainer.querySelectorAll(".tag-chip-remove").forEach((btn) => {
    if (!(btn as any)[BOUND_FLAG]) {
      btn.addEventListener("click", (e) => {
        if (!getTodoFormPermissions().canEditTags) return;
        e.stopPropagation();
...
function updateTagAutocomplete(): void {
  ...
  const matchingTag = getAvailableTags().find((tag) => {
    ...
    return (
      tagLower.startsWith(currentTagLower) &&
