...
export function openWallNoteContextMenu(
  ...
  clientY: number,
  signal: AbortSignal,
  options?: WallNoteMenuOptions,
): Promise<WallNoteMenuChoice | null> {
  return new Promise<WallNoteMenuChoice | null>((resolve) => {
    ...
    let settled = false;
    const finish = (choice: WallNoteMenuChoice | null): void => {
      if (settled) return;
      settled = true;
      cleanup();
      resolve(choice);
    ...
    const listenerOpts = { signal: localAc.signal } as AddEventListenerOptions;
    ...
    const dismissOnOutside = (ev: Event): void => {
